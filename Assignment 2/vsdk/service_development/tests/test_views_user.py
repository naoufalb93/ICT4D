...
class TestUserRegistration(TestCase):
    ...
    def test_user_registration_post_request(self):
        ...
        assert len(
            KasaDakaUser.objects.all()) == number_of_users_pre + 1, 'Number of users should be one greater than before registration.'

        assert KasaDakaUser.objects.all().filter(caller_id=self.caller_id, service=self.voice_service).order_by(
            '-creation_date')[0].service == self.voice_service
